...
// Icon paths are GitHub Octicons 19.38.0 (16px), MIT licensed: https://github.com/primer/octicons
// Colours follow GitHub's pull request state labels.
export const statusStyles: Record<ContributionStatus, StatusStyle> = {
  merged: {
    color: '#8957E5',
    label: 'Merged',
...
export const statuses = Object.keys(statusStyles) as ContributionStatus[];

const HEIGHT = 24;
const ICON_SIZE = 14;

export function statusBadgeWidth(status: ContributionStatus): number {
  return Math.round(34 + statusStyles[status].label.length * 7.4 + 12);
}

// A GitHub-style state pill whose right edge sits at `right` and whose text
// baseline lines up with `baseline`.
export function renderStatusBadge(status: ContributionStatus, right: number, baseline: number, font: string): string {
  const style = statusStyles[status];
  const width = statusBadgeWidth(status);
  const x = right - width;
  const y = baseline - 17;
  const iconScale = ICON_SIZE / 16;
  return `<g class="status-badge">
    <rect x="${x}" y="${y}" width="${width}" height="${HEIGHT}" rx="${HEIGHT / 2}" fill="${style.color}"/>
    <path transform="translate(${x + 11} ${y + (HEIGHT - ICON_SIZE) / 2}) scale(${iconScale})" fill="#FFFFFF" d="${style.path}"/>
    <text x="${x + 31}" y="${y + 16.5}" fill="#FFFFFF" font-family="${font}" font-size="12" font-weight="700">${style.label}</text>
  </g>`;
}
